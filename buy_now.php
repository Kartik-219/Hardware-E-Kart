...
if(isset($_POST['order'])){
    ...
    $check_cart = $conn->prepare("SELECT * FROM `products` WHERE user_id = ?");
    ...
    if($check_cart->rowCount() > 0){
        ...
        foreach ($product_ids as $key => $product_ids) {
            $delete_cart = $conn->prepare("DELETE FROM `cart` WHERE user_id = ? AND id = ?");
            $delete_cart->execute([$user_id, $product_ids]);

            $update_product = $conn->prepare("UPDATE `products` SET quantity = quantity - ? WHERE id = ?");
            $update_product->execute([$quantities[$key], $product_ids]);
        }
